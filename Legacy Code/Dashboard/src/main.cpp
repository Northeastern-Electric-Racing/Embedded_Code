...
/**
 * @brief Initializes the serial console, CAN bus, dashboard pins, and state
 * variables. The three input pins are set trigger on interrupts to simplify the
 * loop() code. The on/off button ISR is triggered when the button is pressed,
 * and both the direction switch and shutdown detection ISRs are triggered on
 * state change.
 */
void setup() {
  Serial.begin(115200);
  ...
  attachInterrupt(digitalPinToInterrupt(SHUTDOWN_DETECTION), shutdownISR,
                  CHANGE);

  pinMode(DIRECTION_SWITCH, INPUT);
  attachInterrupt(digitalPinToInterrupt(DIRECTION_SWITCH), directionISR,
                  CHANGE);
...
/**
 * @brief Continuoulsy reads incoming CAN messages and turns the speaker off
 * after its cooldown.
 */
void loop() {
  myCan.events();
...
/**
 * @brief ISR which triggers when the on/off button is pressed. If precharge
 * isn't complete or the shutdown circuit isn't enabled, nothing happens.
 * Otherwise, the speaker is controlled and an on/off CAN message is sent.
 */
void onOffISR() {
  ...
  if (states.power) { // power on, start speaker
    ...
    sendMessage(CAN_START_MOTOR_ID, 1,
                ON); // Tell pedal box to turn inverter on

    if (states.direction) {
      sendMessage(CAN_DIRECTION_ID, 1,
                  FORWARD); // tell pedal box to set inverter to forward
    } else {
      sendMessage(CAN_DIRECTION_ID, 1,
                  REVERSE); // tell pedal box to set inverter to reverse
    }

  } else { // power off, turn speaker off
    analogWrite(SPEAKER, 0);
    sendMessage(CAN_START_MOTOR_ID, 1,
                OFF); // tell pedal box to turn inverter off
  }
}
...
/**
 * @brief ISR which triggers when the direction switch is toggled. If precharge
 * isn't complete or the shutdown circuit isn't enabled, nothing happens.
 * Otherwise, send a CAN message to set the direction.
 */
void directionISR() {
  if (!states.prechargeComplete || !states.shutdownDetect) {
    return;
...
void incomingCANCallback(const CAN_message_t &msg) {
  if (msg.id == MC_STATES_ID) {
    states.prechargeComplete =
        (msg.buf[0] >> 4) & 1; // true if MC is in wait state
  }
}

...
// the maximum consecutive invalid brake switch readings before throwing an
// error
...
// CAN info
const uint8_t MOTOR_OFF[8] = {0, 0, 0, 0,
                              0, 0, 0, 0}; // message to turn motor off
const uint8_t ACCEL_ERROR[8] = {
    'P', 0, 5, 00,
    0,   0, 0, 0}; // error message of "Vehicle Speed Sensor Malfunction"
const uint8_t BRAKE_ERROR[8] = {
    'P', 0, 5, 04,
    0,   0, 0, 0}; // error message of "Brake Switch A / B Correlation"
const uint8_t BMS_ERROR[8] = {
    1, 1, 1, 1,
    1, 1, 1, 1}; // error message for BMS to activate shutdown circuit
...
void loop() {
  ...
  if ((millis() - lastCommandMessage) >
      COMMAND_MESSAGE_DELAY) { // Adds delay between pedal potentiometer
                               // readings
  ...
  if ((millis() - lastBrakeMessage) >
      BRAKE_MESSAGE_DELAY) { // Adds delay between brake switch readings
    readSwitches();
    lastBrakeMessage = millis(); // reset brake message time
  }

  if ((millis() - lastBMSMessage) >
      BMS_MESSAGE_DELAY) { // Checks if BMS message is being received regularly
    sendMessage(CAN_BMS_SHUTDOWN, 8,
                BMS_ERROR); // triggers shutdown circuit through BMS
  }
}

/**
 * @brief Read the values of the two accelerator pedal potentiometers. Compare
 * them to make sure the values agree and send value to CAN command message.
 *
 * @details The acceleration torque is taken directly from the two
 * potentiometers. The regen torque is calculated as a function of the amount of
 * time the brake has been pressed, along with the car speed (TBD on car speed).
 * If there is any acceleration torque, regen is not factored in. Only when
 * acceleration torque is 0 will regen torque be sent to the MC.
 *
 */
void readPotentiometers() {
  ...
  // Reports errors in potentiometer values if they "bottom" or "top" out or if
  // they are not in sync
  if (accelPin1Val == 0 || accelPin2Val == 0 || accelPin1Val == 1023 ||
      accelPin2Val == 1023 ||
  ...
  if (dischargeEnabled) {
    ...
    int16_t flippedVal =
        (avgVal * -1) + 1023; // reverse it so 0 is when pedal not pressed, and
                              // 1023 is at full press

    if (flippedVal < POT_LOWER_BOUND) { // Set 0 point to prevent a positive
                                        // torque in the resting pedal position
      flippedVal = 0;
    }
  ...
  // calculate regen torque if no accel torque, the BMS says charging is
  // enabled, and the brake is pressed
  if ((appliedTorque == 0) && chargeEnabled && brakePressed) {
    int totalBrakeTime =
        (millis() - timeBrake) /
        1000; // length of time in seconds that the brake has been held down

    if (totalBrakeTime - START_TIME >
        RAMP_TIME) { // max regen torque is allowed
      appliedTorque = -MAX_REGEN_TORQUE;
    } else if (totalBrakeTime - START_TIME > 0) {
      appliedTorque =
          totalBrakeTime *
          (MAX_REGEN_TORQUE / RAMP_TIME); // regen torque depends on how long
                                          // the brake has been held
    } else {
      // do nothing
    }
  }
  ...
  uint8_t message[8] = {
      accelTorqueLowByte, accelTorqueHighByte, 0, 0, isForward, 1, 0, 0};
  sendMessage(
      CAN_MOTOR, 8,
      message); // send message to motor controller with torques and direction
}
...
void readSwitches() {
  ...
  if ((brakePin1Val == LOW) && (brakePin2Val == LOW)) { // both switches are on
    if (!brakePressed) { // set the brake time if the brake has just been
                         // pressed
      timeBrake = millis();
    }
    brakePressed = true;
  ...
  } else if ((brakePin1Val == HIGH) &&
             (brakePin2Val == HIGH)) { // both switches are off
    brakePressed = false;
    currentBrakeErrors = 0;
    ...
    brakePressed = false;
    currentBrakeErrors++;
    if (currentBrakeErrors >=
        MAX_BRAKE_ERRORS) { // send shutdown message, turn off motor, and send
                            // error message
      sendMessage(CAN_BMS_SHUTDOWN, 8, BMS_ERROR);
      sendMessage(CAN_MOTOR, 8, MOTOR_OFF);
...
/**
 * @brief Handles incoming CAN messages. This CAN node acts as the ECU, so
 * incoming messages are use to set the values of the global state variables
 *
 * @param msg received CAN message
 */
void incomingCANCallback(const CAN_message_t &msg) {
  if (msg.id == CAN_START_MOTOR) {
    sendMessage(CAN_MOTOR, 8, MOTOR_OFF); // turns motor off to release lockout
                                          // or because switch is off
    ...
    if ((msg.buf[4] != 0) || ((msg.buf[5] >> 3) & 1) ||
        ((msg.buf[5] >> 4) &
         1)) { // only send message for fault bits 32-39, 43, 44
      sendMessage(CAN_BMS_SHUTDOWN, 8,
                  BMS_ERROR); // triggers shutdown circuit through BMS
    }
    ...
    if (((msg.buf[0] >> 5) & 1) ||
        ((msg.buf[0] >> 6) &
         1)) { // only send message for cell voltage too high/low faults
      sendMessage(CAN_BMS_SHUTDOWN, 8,
                  BMS_ERROR); // triggers shutdown circuit through BMS
    }
  }

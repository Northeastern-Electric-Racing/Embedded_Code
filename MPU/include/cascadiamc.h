...
private:
  ...
  union {
    ...
    struct {
      int16_t accelTorque : 16; // bitfield might not be necessary, but I wanted
                                // to be explicit
  ...
  /**
   * @brief disables Motor Controller Lockout by sending motor controller off
   * message
   *
   */
...
public:
  ...
  /**
   * @brief Writes the desired state of the motor controller using the saved
   * config
   * @note Config should be fully loaded to minimize message clutter (i.e. try
   * not to send the message too often)
   */
  void writeMCState();

...
CascadiaMC::CascadiaMC() { motorCommand_wait.cancelTimer(); }
...
void CascadiaMC::disableMCLockout() {
  ...
  sendMessageCAN1(CANMSG_ACCELERATIONCTRLINFO, 8,
                  mcOff); // release lockout / OFF
...
bool CascadiaMC::getIsOn() { return mcMsg.config.isOn; }

bool CascadiaMC::getDirection() { return mcMsg.config.isForward; }

void CascadiaMC::changeTorque(uint16_t p_accelTorque) {
  mcMsg.config.accelTorque = p_accelTorque;
}

uint16_t CascadiaMC::getTorque() { return mcMsg.config.accelTorque; }
...
bool CascadiaMC::checkFault() { return isFaulted; }
...
bool CascadiaMC::isMotorMoving() { return abs(motorSpeed) > 0; }

bool CascadiaMC::shouldMotorBeSpinning() {
  return mcMsg.config.accelTorque > NOT_SPINNING_TORQUE_LIMIT;
}

int16_t CascadiaMC::getMotorSpeed() { return motorSpeed; }

void CascadiaMC::setRadiatorTemp(int16_t temp) { radiatorTemp = temp; }

int16_t CascadiaMC::getRadiatorTemp() { return radiatorTemp; }
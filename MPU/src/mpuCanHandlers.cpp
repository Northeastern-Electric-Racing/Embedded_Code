...
void bmsStatus_cb(const CAN_message_t &msg) {
  ...
  bms.setFaultStatus((msg.buf[1]) << 24 | (msg.buf[2]) << 16 |
                     (msg.buf[3]) << 8 | (msg.buf[4]));
...
void bmsCurrents_cb(const CAN_message_t &msg) {
  ...
  if (dischargeCurrentLimit > 1000)
    dischargeCurrentLimit =
        (msg.buf[1] << 8) |
        msg.buf[0]; // MPU seems to be randomly flipping CAN byte order??
  bms.setCurrentLimit(dischargeCurrentLimit);
...
void mpuCanCallback(const CAN_message_t &msg) {
  switch (msg.id) { // using a switch statement for set CAN IDs

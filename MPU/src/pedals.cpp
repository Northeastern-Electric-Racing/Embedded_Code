...
FaultStatus_t Pedals::readAccel() {
  ...
  // If the pedal reading process is NOT finished, return NOT_FAULTED because it
  // hasn't finished collecting data
  if (pedalVal == NOT_DONE_READING)
    return NOT_FAULTED;

  // If the pedal reading process has detected a FAULT, return FAULT
  if (accelerator.isFaulted() == FAULTED)
    return FAULTED;

  // Calculate percentage value for how far down the accelerator is pressed
  int16_t flippedVal =
      (pedalVal * -1) + 1023; // reverse it so 0 is when pedal not pressed, and
                              // 1023 is at full press

  // Cleansing Value
  // Note: POT_LOWER_BOUND is nonzero to allow for some mechanical give while
  // not exerting torque
  if (flippedVal < POT_LOWER_BOUND)
    flippedVal = 0;
  if (flippedVal > POT_UPPER_BOUND)
    flippedVal = POT_UPPER_BOUND;

  double multiplier = ((double)flippedVal - ACCEL_OFFSET) /
                      650; // torque multiplier from 0 to 1;

  // Calculate the correct amount of torque to command using precentage pressed
  // value
  ...
  if (drive_state == PIT || drive_state == REVERSE) {
    uint16_t newVal;
    // Results in a value from 0.5 to 0 (at least halving the max torque at all
    // times in pit or reverse)
...
#endif
  // Return the current fault status of the accelerator, send fault if it wasn't
  // caught earlier
...
FaultStatus_t Pedals::readBrake() {
  ...
  // If the pedal reading process has detected a FAULT, return FAULT
  if (brakes.isFaulted() == FAULTED)
    return FAULTED;

  // Set brakePressed to whether or not the brake value is greater than the
  // braking threshold
...
int16_t Pedals::calcCLTorqueLimit() {
  ...
  // calculated = (0.9 * (CL_TO_TOQRUE_CONST * (dcVoltage / 10) * dcCurrent)) /
  // (motorSpeed + 1);
  calculated =
      (dcCurrent * dcVoltage * sqrt(3)) / (motorSpeed * CL_TO_TOQRUE_CONST);

...
#define SWITCHBYTES(x)                                                         \
  (((x << 8) | (x >> 8)) & 0xFFFF) // Switches the first and second byte (This
                                   // is required for all command values)

#define SHT30_I2C_ADDR 0x44 // If ADDR (pin2) is connected to VDD, 0x45

#define SHT30_START_CMD_WCS                                                    \
  0x2C06 // Start measurement command with clock streching enabled and high
         // repeatability
#define SHT30_START_CMD_NCS                                                    \
  0x2400 // Start measurement command with clock streching disabled and high
         // repeatability
...
public:
  ...
  /**
   * @brief Allows for heating element control, bool = true->enables,

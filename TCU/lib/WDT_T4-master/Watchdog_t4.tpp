...
WDT_FUNC void WDT_OPT::begin(WDT_timings_t config) {
  IOMUXC_GPR_GPR16 |=
      0x200000; /* undocumented register found by PaulS to fix reset */
  uint32_t nvicIRQ = 0;
  ...
  if (_device == WDT3) {
    ...
    if (config.clock == LPO_CLK ||
        config.clock == INT_CLK) { /* LPO_CLOCK & INT_CLOCK are 32KHz */
      ...
      config.timeout = constrain(config.timeout, lowest_limit_without_prescaler,
                                 highest_limit_with_prescaler);
      config.window = constrain(config.window, lowest_limit_without_prescaler,
                                highest_limit_with_prescaler);
      if (config.timeout <
          highest_limit_without_prescaler) { /* prescaler not needed */
        toVal = (config.timeout / (1.0f / 32.0f));
        if (config.window)
          config.window = (config.window / (1.0f / 32.0f));
      } else { /* use prescaler */
        preScaler = 1;
    ...
    WDOGb_CS(_device) = ((config.window) ? WDOG_CS_WIN : 0) |
                        ((preScaler) ? WDOG_CS_PRES : 0) | WDOG_CS_FLG |
                        (config.update << 5) | (config.cmd32en << 13) |
                        (config.clock << 8) |
                        ((config.callback) ? WDOG_CS_INT : 0) | WDOG_CS_EN;
    __enable_irq();
  ...
  WDOGb_CS(_device) = WDOG_CS_FLG | (config.update << 5) |
                      (config.cmd32en << 13) | (config.clock << 8) |
                      ((config.callback) ? WDOG_CS_INT : 0) | WDOG_CS_EN;
  config.timeout =
      constrain(config.timeout, 0.5f, 128.0f); /* timeout to reset */
  config.timeout = (config.timeout - 0.5f) / 0.5f;
  WDOGb_WCR(_device) =
      WDOG_WCR_SRS |
      WDOG_WCR_WT((uint8_t)config.timeout); /* do NOT negate the reset signal
                                               when clearing register */
  config.trigger = constrain(config.trigger, 0.0f,
                             127.5); /* callback trigger before timeout */
  config.trigger /= 0.5f;
  WDOGb_WICR(_device) =
      ((config.callback) ? WDOG_WICR_WIE : 0) | WDOG_WICR_WTIS |
      WDOG_WICR_WICT(
          (uint8_t)config.trigger); /* enable interrupt, clear interrupt */
  WDOGb_WCR(_device) |= WDOG_WCR_WDE |
                        ((config.lp_suspend) ? WDOG_WCR_WDZST : 0) |
                        WDOG_WCR_WDA | WDOG_WCR_WDT | WDOG_WCR_SRE;
  WDOGb_WMCR(_device) = 0; /* Disable power down counter, else GPIO will force
                              LOW indefinately after 16 seconds */
  NVIC_ENABLE_IRQ(nvicIRQ);
}

WDT_FUNC bool WDT_OPT::expired() {
  if (WDT3 == _device)
    return SRC_SRSR & SRC_SRSR_WDOG3_RST_B;
  if (EWM == _device)
    return 0; /* No status register? */
  return WDOGb_WRSR(_device) & WDOG_WRSR_TOUT;
}

WDT_FUNC void WDT_OPT::reset() {
  if (WDT3 == _device || EWM == _device)
    SCB_AIRCR = 0x05FA0004; /* WDT3 & EWM doesn't have a reset register, fall
                               back to ARM */
...
WDT_FUNC void WDT_OPT::feed() {
  ...
  if (_device == WDT1 || _device == WDT2) {
    ...
    if (WDOGb_CS(_device) & WDOG_CS_CMD32EN)
      WDOGb_CNT32(_device) = 0xB480A602;
    else {
...
WDT_FUNC void WDT_OPT::watchdog_isr() {
  if (watchdog_class_handler)
    watchdog_class_handler();
  if (WDT3 == _device)
    WDOGb_CS(_device) |= WDOG_CS_FLG;
  else
    WDOGb_WICR(_device) |= WDOG_WICR_WTIS;
...
WDT_FUNC void WDT_OPT::ewatchdog_isr() {
  if (watchdog_class_handler)
    watchdog_class_handler();
  EWM_CTRL &= ~(1U << 3); /* disable further interrupts, EWM has been triggered,

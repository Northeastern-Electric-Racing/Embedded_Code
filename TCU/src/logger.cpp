/**
 * The standard block size for SD card transfers is 512 bytes. Each write
 * operation to the SD card should be as close to this size as possible, as the
 * remaining space will be padded and used anyway.
 *  - each message is a max of 49 bytes, so a buffer size on multiples of 10 is
 * ideal
 *
 * The Teensy Loader automatically syncs the RTC to the PCs system time on
 * upload, so the time library can be used for real time access if a coin cell
 * is connected to VBAT on the teensy.
 *
 * All RTC times are in units of seconds since Jan 1st 1970, which is an
 * unsigned long.
 *   - We also keep track of the millis() of the car for extra precision on time
 * stamps
 *
...
#define MAX_BUFFERED_MESSAGES                                                  \
  50 // max number of buffered CAN messages before logging to SD card
...
// Buffer information (use 2 buffers to prevent overwrites during logging
// delays)
int buf1Length = 0;
int buf2Length = 0;
...
/**
 * @brief Sets the logging file name using the format log-NUM.txt, where NUM is
 * either the current unix time if using the RTC or a valid numeric index
 * starting from 0.
 *
 */
 ...
 */
static void reset() {
  noInterrupts() initialized = false;
  buf1Length = 0;
  buf2Length = 0;
...
bool LoggerActive() { return initialized; }
...
LOGGER_STATUS LoggerWrite() {
  ...
  if ((millis() - lastLogTime) <= minLogFrequency &&
      bufLength < MAX_BUFFERED_MESSAGES) {
  ...
  if (logFile) {
    ...
    // clear bufLength variables (must clear the opposite of whatever one is in
    // use)
    if (usingBuf1) {

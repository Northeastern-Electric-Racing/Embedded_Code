...
void SHT30::SHT30write(
    cmd_t *cmd, uint8_t numBytes) // Allows for private I2C writing with SHT30
{
...
bool SHT30::SHT30read(
    uint8_t *msg, uint8_t numBytes) // Allows for private I2C reading with SHT30
{
...
void SHT30::enableHeater(
    bool h) // Allows for heating element control, bool = true->enables,
            // false->disable. Clears built up condensation
{
  cmd_t cmd = {.cmdVal = h ? SWITCHBYTES(SHT30_HEATEREN)
                           : SWITCHBYTES(SHT30_HEATERDIS)};
  SHT30write(&cmd, 2);
}

void SHT30::getTempHumid(
    uint8_t *msg) // Public function to start a measurement and print values
{
